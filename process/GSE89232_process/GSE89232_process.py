...
raw_data_file = open(inpath + 'GSE89232_expMatrix.txt')
...
mapping_exp_name = {'blood_bdca3':'CD141+ cDC',
                    'blood_bdca1': 'CD1c+ cDC',
                    'cb_predc': 'CB pre-cDC',
                    'blood_predc': 'Blood pre-cDC'
                    }
for idx, exp in enumerate(Exp_Name_list):
    exp_split = exp.split('_')
    exp_num = exp_split[-1]
    exp_class = '_'.join(exp_split[:-1])
    new_exp = mapping_exp_name[exp_class] + '_' + exp_num
    found = False
    for idx_title, title in enumerate(aux['Title']):
        if new_exp == title:
            found = True
            break
    cell_type =  title.split('_')[0]
